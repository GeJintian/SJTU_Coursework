        ...
        if (cache_write == 1'b0) begin //read
            ...
            // end hit
                ...
                if (cache[address_cpu[5:4]][132] == 1'b1) begin
                    ...
                    //$display("Before, mem_write is %b",mem_write);
                    #0.1 mem_write = 1'b0;
                    // reset dirty bit
                    cache[address_cpu[5:4]][132] = 1'b0;
                // end dirty
                end
                //$display("After, mem_write is %b",mem_write);
                address_mem = address_cpu;
                //#0.1    $display("Data is %H", data_read4cache);
                //$display("Address is %b", address_mem);
                // overwrite the block with new value
                #0.1 cache[address_cpu[5:4]][127:0] = data_read4cache;
                //$display("Data is %H", data_read4cache);
                // update tag
                cache[address_cpu[5:4]][131:128] = address_cpu[9:6];
                // read word
                case (address_cpu[3:2])
                    2'b00: data_read4cpu = cache[address_cpu[5:4]][31:0];
                    2'b01: data_read4cpu = cache[address_cpu[5:4]][63:32];
